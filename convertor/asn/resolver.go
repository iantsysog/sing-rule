...
import (
	...
	"github.com/iantsysog/sing-rule/constant"
...
func (r *ASNResolver) normalizeASN(asn string) string {
	trimmed := strings.TrimSpace(asn)
	if len(trimmed) >= 2 {
		if strings.EqualFold(trimmed[:2], "AS") {
			trimmed = strings.TrimSpace(trimmed[2:])
		}
		if len(trimmed) >= 2 && strings.EqualFold(trimmed[len(trimmed)-2:], "AS") {
...
func (r *ASNResolver) fetchJSON(ctx context.Context, url, apiName string, target any) error {
	...
	if err != nil {
		return E.Cause(err, "read ", apiName, " response")
	}
	if len(body) > maxResponseBodyBytes {
		return E.New(apiName, " response exceeds max allowed size")
	}
...
func (r *ASNResolver) ResolveASN(ctx context.Context, asn string) ([]string, error) {
	...
	resolvedAny, err, _ := r.inflight.Do(asnID, func() (any, error) {
		...
		prefixes, resolveErr := r.resolveASNFromProviders(ctx, asnID)
		if resolveErr != nil {
...
func (r *ASNResolver) resolveASNFromProviders(ctx context.Context, asnID string) ([]string, error) {
	prefixes, _ := r.fetchFromBGPView(ctx, asnID)
	if len(prefixes) > 0 {
		return prefixes, nil
	}

	prefixes, _ = r.fetchFromRIPE(ctx, asnID)
	if len(prefixes) > 0 {
		return prefixes, nil
	}
