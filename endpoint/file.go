...
func (f *FileEndpoint) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		f.logger.Error("request failed ", r.RemoteAddr, " - ", r.UserAgent(), " \"", r.Method, " ", r.URL, " ", r.Proto, "\": ", err)
		return
	}
	f.logger.Debug("request served ", r.RemoteAddr, " - ", r.UserAgent(), " \"", r.Method, " ", r.URL, " ", r.Proto, "\"")
}
...
func (f *FileEndpoint) serve(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	cacheKey := F.ToString("file.", f.endpointIndex, ".", cachePath)
	...
	if f.convertRequired {
		...
		rules, convertErr := f.sourceConvertor.From(requestCtx, response.Content, convertOptions)
		if convertErr != nil {
			return http.StatusInternalServerError, E.Cause(convertErr, "decode source")
		}
		binary, convertErr = f.targetConvertor.To(requestCtx, rules, convertOptions)
		if convertErr != nil {
...
func (f *FileEndpoint) writeCache(w http.ResponseWriter, cachedBinary *adapter.SavedBinary, convertOptions adapter.ConvertOptions) error {
	...
	if !cachedBinary.LastUpdated.IsZero() {
		header.Set("Last-Modified", cachedBinary.LastUpdated.UTC().Format(http.TimeFormat))
	}

	if _, err := w.Write(cachedBinary.Content); err != nil {
